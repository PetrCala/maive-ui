...
- **Does:** cut runaway compute automatically to a trickle (two slots,
  ~$0.42/hr worst case) and bound each abuse episode to a few dollars
...
- Raise `lambda_r_backend_reserved_concurrency` (and
  `orchestrator_maximum_concurrency` together) if legitimate traffic grows and
  the throttle alarm fires on real load. Each unit is ~$0.21/hr (~$150/month)
  of worst-case exposure at the current 3538 MB.
...
load pattern that looks like abuse. Before one:
...
- About 10 minutes before the session, run `npm run cloud:warm`
  ...
  which holds each container for 3 s so all 15 end up as separate warm
  containers and the first wave of runs skips the ~5 s R cold start. It also
  sends 20 concurrent requests to the UI; that part is best effort, because a
  route that pins instances on the public UI would be an abuse vector. Lambda
  reclaims idle containers after several minutes, so rerun it after a long gap.

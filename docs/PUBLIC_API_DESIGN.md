...
| D6 | **All parameters optional, resolved by the one resolver the browser uses** (`src/lib/parameterResolver.ts`, #555), with data-dependent defaults (study clustering follows `study_id`, WLS runs with standard weights) and named recipes; every 200 echoes `resolvedParameters`. Unknown keys and conflicting values are a 400. | A minimal valid request is just `{"data": [...]}`. A human in the browser and an API caller asking for the same thing on the same data must get the same analysis, and be told what ran. |

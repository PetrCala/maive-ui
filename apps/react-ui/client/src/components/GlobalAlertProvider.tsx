...
export const useGlobalAlert = () => {
  ...
  if (!ctx) throw new Error("useGlobalAlert must be used within GlobalAlertProvider")
  return ctx
}
...
export const GlobalAlertProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const showAlert = useCallback((msg: string, lvl: AlertLevel = "info", dur = 2500) => {
    setMessage(msg)
    setType(lvl)
    setDuration(dur)
    setOpen(true)
  ...
  return (
    <GlobalAlertContext.Provider value={{ showAlert }}>
      {children}
      <AlertPopup message={message} type={type} open={open} onClose={handleClose} duration={duration} />

  ...
  const rows =
    parameters.modelType === "RTMA"
      ? [
          ...
          ["Alpha Select", String(RTMA_ALPHA_SELECT)],
          ["CI Level", String(RTMA_CI_LEVEL)],
...
export function generateVersionManifest(
  ...
  const rSourceLines = packagedRSourceFiles(parameters.modelType)
    ...
    .join("\n");
  // The page carries RTMA runs in the MAIVE-shaped parameter object, so an
  // RTMA manifest used to list MAIVE settings (method, weights, first stage)
  // the run never had. Each model lists the settings its script runs with.
  const analysisConfiguration: Array<[string, string]> = isRtma
    ? [
        ["Model Type", "RTMA"],
        ["Favor Positive", String(parameters.favorPositive)],
        ["Alpha Select", String(RTMA_ALPHA_SELECT)],
        ["CI Level", String(RTMA_CI_LEVEL)],
        ...
        [
          "Sampler Seed",
          rtmaSeed == null
            ? `${RTMA_FALLBACK_SEED} (default; the original run recorded no seed, so its credible intervals cannot be reproduced exactly)`
            : String(rtmaSeed),
        ],
      ]
    : [
        ...
        ["Use Instrumenting", String(parameters.shouldUseInstrumenting)],
        ["Study Dummies", String(parameters.includeStudyDummies)],
        ["Study Clustering", String(parameters.includeStudyClustering)],
        ["SE Treatment", parameters.standardErrorTreatment],
        ["Anderson-Rubin", String(parameters.computeAndersonRubin)],
        ["Log First Stage", String(parameters.useLogFirstStage)],
        ["Winsorize", `${parameters.winsorize}%`],
      ];
  const analysisLines = analysisConfiguration

...
cat("mu 95% CI:   ", sprintf("[%.6f, %.6f]", results$muCI[1], results$muCI[2]), "\\n")

cat("\\n=== HETEROGENEITY ===\\n")
cat("tau (mode):  ", sprintf("%.6f", results$tau), "\\n")
cat("tau 95% CI:  ", sprintf("[%.6f, %.6f]", results$tauCI[1], results$tauCI[2]), "\\n")

cat("\\n=== NONAFFIRMATIVE ESTIMATES ===\\n")
cat("Count:       ", results$nonaffirmativeCount, "\\n")
cat("Proportion:  ", sprintf("%.1f%%", results$nonaffirmativeProportion * 100), "\\n")

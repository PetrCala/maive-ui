...
run_maive_model <- function(data, parameters) {
  ...
  # Weight options: equal weights=0, standard weights=1, adjusted weights=2, study weights=3
  # Study weights: w_ij = 1/n_j where n_j is the number of estimates in study j
  # Note: This assumes the MAIVE package supports weight=3 for study weights.
  # If not, custom implementation may be required (pre-compute weights and modify data).
  ...
  # Check if weight switch returned NULL (no match found)
  if (is.null(weight)) {
    cli::cli_abort(paste("Invalid weight value:", params$weight))
  }

  # For study weights, we need to ensure study_id is present
  if (weight == 3 && !("study_id" %in% names(df))) {

...
resource "aws_iam_policy" "data_bucket_rw" {
  ...
  policy = jsonencode({
    Version = "2012-10-17",
    Statement = [
      { Effect = "Allow", Action = ["s3:GetObject", "s3:PutObject", "s3:DeleteObject"], Resource = "${data.terraform_remote_state.foundation.outputs.data_bucket_name}/*" },
      { Effect = "Allow", Action = ["s3:ListBucket"], Resource = data.terraform_remote_state.foundation.outputs.data_bucket_name }
    ]
  })
}
